        ...
        <swiper-slide *ngFor="let s of slides; trackBy: trackId; let i = index">
          <div class="relative w-full h-full select-none">
            ...
            <div class="absolute inset-x-0 bottom-0 p-8 pointer-events-none">
              ...
              >
                <h2
                  class="font-display text-3xl md:text-5xl leading-tight transition-all duration-500 ease-out delay-75
                ...
                <p
                  class="mt-2 text-neutral-200/80 transition-all duration-500 ease-out delay-150
                        opacity-0 translate-y-2"
                  [class.opacity-100]="activeIdx() === i"
                  [class.translate-y-0]="activeIdx() === i"
                >
...
export class HeroCarouselComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private prefetchAround(activeIndex: number) {
    ...
    const sNext = this.slides[next];
    const sNN = this.slides[nn];
    const u1 = sNext?.url ?? (sNext as any)?.storage_path ?? '';
    const u2 = sNN?.url ?? (sNN as any)?.storage_path ?? '';
    if (u1) this.prefetchImage(u1);
    if (u2) this.prefetchImage(u2);
  }
}
