  ...
  <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
    <a *ngFor="let p of posts"
      ...
      <div class="relative aspect-video overflow-hidden">
        ...
        />
        <ng-template #noCover>
          <div class="h-full w-full grid place-items-center text-sm text-white/60 bg-white/5">No cover</div>
        </ng-template>

        <div class="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/60 via-black/0 to-transparent"></div>
      ...
      <div class="p-4">
        <h2 class="font-semibold leading-snug line-clamp-2 group-hover:text-white transition-colors">
          {{ p.title }}
        </h2>
        <div class="mt-2 flex items-center justify-between text-xs text-white/60">
          ...
          <span class="inline-flex items-center gap-1 opacity-80 group-hover:opacity-100">
            Leer
            <svg class="w-3.5 h-3.5 transition-transform group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="currentColor">
...
export default class BlogPage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.posts = await firstValueFrom(this.http.get<BlogPost[]>(apiUrl('/blog'))); // 👈
