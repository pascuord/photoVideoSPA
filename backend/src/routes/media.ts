...
router.get('/media', async (req, res) => {
  try {
    ...
    const [byId, bySlug] = await Promise.all([
      supabase.from('blog_posts').select('slug, linked_media_id').in('linked_media_id', ids),
      supabase.from('blog_posts').select('slug, linked_media_slug').in('linked_media_slug', slugs),
    ]);
